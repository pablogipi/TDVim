...
if !exists('g:timeStampAuthorLeader')
	let s:timeStampAuthorLeader = 'Modified By: '
else
	let s:timeStampAuthorLeader = g:timeStampLeader
endif
...
function! UpdateWithLastMod()
	...
	if search(s:timeStampLeader) <= 20 && &modifiable
		...
		let newdate = strftime("%A, %d %B %Y %X").' '.timezone
		if lastdate == newdate
			exe pos
			return
		end
