...
" ConvertWindowsToPosixPath {{{2
function! ConvertWindowsToPosixPath(windows_path)
    " Replace backslashes with forward slashes
    let posix_path = substitute(a:windows_path, '\\', '/', 'g')

    " Convert drive letter (e.g., C:) to /c/
    "let posix_path = substitute(posix_path, '^\([A-Za-z]\):', '/\1', '')
    " Remove drive letter
    let posix_path = substitute(posix_path, '^\([A-Za-z]\):', '', '')

    " Convert UNC paths (e.g., \\server\share to //server/share)
    let posix_path = substitute(posix_path, '^//', '/', '')
...
function! s:SetLocalPath ()
    ...
    "if g:tdvim_dev_mode && has('b:tdvim_proj_root')
    if g:tdvim_dev_mode
...
" VSetSearch {{{2
" Visual mode search
" From: https://github.com/godlygeek/vim-files/blob/master/plugin/vsearch.vim
...
" FindWindowByType {{{2
" Given a buffer type, find it window number
" This can be used for different porpose, detect a particualar buffer type is
" open in a window
" If buffer type is not found then return -1, otherwise return the index of the
" window
" Check against:
" - Buffer type
" - File Type
" - Preview Window
"   TODO: use this function in DWM
function! TDVimFindWindowByType ( buffertype )
  " Save current window number to revert.
...
function! TDVimTabAutocompleteWrapper( direction)
    ...
    if pumvisible()
        " If menu is visible then just move up and down. If use C-P and C-N
        " then the selection will be applied to the text, we prefer to just
        " move around and manuyally select and insert pressin enter on
...
" CleanupNERDTreeBuffer {{{2
" Define a function to clean up NERDTree buffers
function! s:CleanupNERDTreeBuffer()
    " If we are currently in a NERDTree window, do nothing
    "if &filetype ==# 'nerdtree'
        "return
    "endif
    " Get all buffers
    for buf in range(1, bufnr('$'))
        " Check if buffer is a NERDTree buffer (exists, loaded, and has nerdtree filetype)
        if bufexists(buf) && bufloaded(buf) && getbufvar(buf, '&filetype') == 'nerdtree'
            " Check if we're not currently in this buffer
            if buf != bufnr('%')
                " Wipe out the NERDTree buffer
...
function! TDVimLightlineFilename() abort
    ...
    if &filetype == "help" || &filetype == "ctrlp" || &filetype == "tagbar" || &filetype == "nerdtree" || &filetype == "scratch"
        return ''
    endif
...
function! TDVimGitHunksummary()
    ...
    if g:tdvim_dev_mode && exists("*GitGutterGetHunkSummary")
...
function! TDVimLightlineExtraInfo() abort
  ...
  if &filetype == "qf"
      ...
      let l:extra_info = ''
      " Get closest function using Vista
      if exists('b:vista_nearest_method_or_function') 
          "let l:extra_info = "\uf794⯈" . b:vista_nearest_method_or_function . '(...)'
          "let l:extra_info = "\uf794\u2192" . b:vista_nearest_method_or_function . '(...)'
          let l:extra_info = "→ " . b:vista_nearest_method_or_function . '(...)'
          "let l:extra_info = "\uf794\u27a4" . b:vista_nearest_method_or_function . '(...)'
          "let l:extra_info = "\u27a4" . b:vista_nearest_method_or_function . '(...)'
...
" Restore session {{{3
" To debug session restore dont use silent! when calling this function
" By default the autocommand  uses silent
fu! TDVimRestoreSess()
    " Only in project mode
    if !g:tdvim_dev_mode || !exists('g:tdvim_proj_root')
        return
    ...
    if filereadable(sessionfile)
        ...
        " them on load
        " Also remove any buffer not relative to our current project
        for nbuf in range(1, bufnr('$'))
            let to_delete = 0
            "echomsg "Buffer number: " . nbuf
            let buffer_name = bufname( nbuf )
            let buffer_path = expand('#' . nbuf . ':p')
            echomsg "Buffer: " . buffer_path
            if bufexists(nbuf) && !buflisted(nbuf)
                continue  " Skip unlisted buffers (optional)
            elseif isdirectory(buffer_name)
                let to_delete = 1
            "elseif match(buffer_name, "NERD_tree") == 0
            elseif buffer_name =~ "^NERD_tree"
                let to_delete = 1
            elseif ConvertWindowsToPosixPath(buffer_path) !~ ConvertWindowsToPosixPath(g:tdvim_proj_root)
                let to_delete = 1
                echomsg "Buffer " . buffer_path . " doesn't match project root: " . g:tdvim_proj_root
            endif
            if to_delete == 1
                echomsg("Buffer deleted: " . buffer_name)
                exe "silent! bdelete! " . nbuf
            endif
        endfor
    else
...
" Mappings
    " LspDefinition
    " LspPeekDefinition
    " LSPNext/Prev Errors/Diasnostics
    " LspFormat
    " LspDiasnostics
    " LspReferences
func! s:TDVimLSPMappings()
    nnoremap <silent> <buffer> <c-]>    :LspDefinition<CR>
    nnoremap <silent> <buffer> gd       :LspDefinition<CR>
    nnoremap <silent> <buffer> K        :LspHover<CR>
    nnoremap <silent> <buffer> \        :LspPeekDefinition<CR>
    nnoremap <silent> <buffer> <C-F6>   :LspDocumentDiagnostics<CR>
    nnoremap <silent> <buffer> <S-F6>   :LspReferences<CR>
    nnoremap <silent> <buffer> <]s>     :lnext<CR>
    nnoremap <silent> <buffer> <[s>     :lprev<CR>
endfunc
" }}}

" TDVimHighlightWordUnderCursor {{{2
" Highlight word under cursor, from: https://stackoverflow.com/questions/25227281/how-to-auto-highlight-the-current-word-in-vim
function! TDVimHighlightWordUnderCursor()
    ...
    if getline(".")[col(".")-1] !~# '[[:punct:][:blank:]]'
        ...
        hi MatchWord cterm=underline gui=underline guibg=#3b404a
...
" If buffer is already in diff mode then remove HEAD buffer and unset diffmode
" Use GitGutterDiff command to generate the diff with HEAD
function! TDVimGitDiff ()
    if &diff
        " If we aare in diff mode then remove it and HEAD buffer
        diffoff
        " 2. Loop through all listed buffers
        for bufnr in filter(range(1, bufnr('$')), 'buflisted(v:val)')
            " 3. Check for the name 'HEAD'
            if bufname(bufnr) ==# 'HEAD'
                " 4. Get the window ID for that buffer to check its diff status
                let bwinid = bufwinid(bufnr)
                if bwinid != -1 && getwinvar(bwinid, '&diff')
                    " 5. Wipe it out silently
                    execute 'silent bdelete ' . bufnr
                    echo "HEAD diff buffer removed."
                endif
            endif
        endfor
    else
...
" Highlight certain keywords, extend TODO, FIXME and XXX
" myTodo is defined in autcommands
...
" NOTE: netrw_gitignore#Hide() has disappear in recent version of vim
...
command! TDVimMakeTags !ctags -R  --sort=1 --c++-kinds=+p --python-kinds=-iv --fields=+iaSl --extras=+q --exclude='.git' --exclude='.svn' --exclude='*.so' --exclude='__pycache__' --exclude='*.pyc'  
...
" Quick TDvim Help. Cheat sheet
nnoremap  <silent> <C-F1>             :call TDVimShowQuickHelp()<CR>
xnoremap  <silent> <C-F1>             :call TDVimShowQuickHelp()<CR> 
inoremap  <silent> <C-F1>             <C-O>:call TDVimShowQuickHelp()<CR> 
" }}}
...
" Grep In project sending results to Quickfix
nnoremap  <C-F4>          :TDvimGrep <C-R><C-W>
inoremap  <C-F4>          :<C-O>TDvimGrep  <C-R><C-W>
xnoremap  <C-F4>          :TDvimGrep  <C-R><C-W>

" Start interactive EasyAlign in visual mode (e.g. vipga)
nmap <silent> <leader>a <Plug(LiveEasyAlign)>
vmap <silent> <Enter>   <Plug>(LiveEasyAlign)
...
" Navigate through changes and jumps
" Navigate Changes
" Default mapped to []d
" XXX: []c is used in diffmode, change to []d
nmap <silent> [d    g;
imap <silent> [d    <Esc>g;
vmap <silent> [d    g;
nmap <silent> ]d    g,
...
vmap <silent> ]d    g,
if s:has_fzf
    nnoremap   <F7>   :Changes<cr>
    vnoremap   <F7>   :Changes<cr>
    inoremap   <F7>   <ESC>:Changes<cr>
    nnoremap   <leader>fc   :Changes<cr>
    vnoremap   <leader>fc   :Changes<cr>
    inoremap   <leader>fc   <ESC>:Changes<cr>
endif
...
nmap <silent> ]j    <C-I> 
if s:has_fzf
    nnoremap   <S-F7>   :Jumps<cr>
    vnoremap   <S-F7>   :Jumps<cr>
    inoremap   <S-F7>   <ESC>:Jumps<cr>
    nnoremap   <leader>fj   :Jumps<cr>
    vnoremap   <leader>fj   :Jumps<cr>
    inoremap   <leader>fj   <ESC>:Jumps<cr>
endif

" Navigate Lines
" Default mapped to <leader>fl
if s:has_fzf
    nnoremap   <leader>fl   :Lines<cr>
    vnoremap   <leader>fl   :Lines<cr>
    inoremap   <leader>fl   <ESC>:Lines<cr>
endif
" }}}
...
" ]t is usually overriden by more advanced methods, like Jedi or a LS
nnoremap   ]t   :tjump <C-r><C-w><CR>
nnoremap   [t   <C-t>
" Inspect and preview tag
" DEPRECATED: weFriday, 10 Oct 2025 01:05:02on't use tags anymore
"nnoremap   <S-F6>   :ptjump <C-r><C-w><space>
"xnoremap   <S-F6>   :ptjump <C-r><C-w><space>

" Insert Timestamp
nmap <leader>d i<C-R>=strftime("%A, %d %b %Y %r")<CR><Esc>

" Autocomplete using Previous and Next
inoremap   <TAB>    <C-R>=TDVimTabAutocompleteWrapper("forward")<CR>
inoremap   <S-TAB>  <C-R>=TDVimTabAutocompleteWrapper("backward")<CR>
...
" Terminal {{{2
" Termnal keymaps
" nnoremap <F11>       :wincmd b \| botright terminal<CR>
" Open terminal at bottom
nnoremap <F11>       :TDVimOpenTerminal<CR>
nnoremap <leader>t  :TDVimOpenTerminal<CR>
" Minimize terminal window and jump to previous window
tnoremap <F11>       <C-W>1_<C-W>W
" }}}
...
" This will be deprecated but is still needed to get a proper fuzzy search for
" keywords
...
if has("autocmd")
    ...
    augroup tdvimAftertBufferRead
        ...
        " Show guides for TAB, quick and better indentation guides
        ...
        " setlocal list conceals leading and trailing charatcers so it cancels
        " the ExtraWhitespace option set before
        " Show indent guides
        autocmd BufEnter  *.py,*.pyw,*.c,*.cpp,*.vim,*.h,*vimrc setlocal list
        autocmd BufEnter  *.py,*.pyw,*.c,*.cpp,*.vim,*.h,*vimrc execute 'setlocal listchars=trail:·,tab:│\ ,leadmultispace:┆' . repeat('\ ', &sw - 1)
        " After reading buffer cleanup any existing Nerdtree Buffer
        autocmd BufReadPost * call s:CleanupNERDTreeBuffer()
    augroup END
    " BufEnter
...
"let g:lsp_settings_filetype_python = [ 'ruff-lsp' ] " Ruff works great as linter, fast. But only this option alone doesn't provide code completion
"let g:lsp_settings_filetype_python = [ 'jedi-language-server' ] " This one is promising, linter is not gret but completion is non blocking and works
let g:lsp_settings_filetype_python = [ 'jedi-language-server', 'ruff-lsp'  ] " Mixing, jedi for completions/symbols and ruff as static linter/diagnostics
"set foldmethod=expr
  "\ foldexpr=lsp#ui#vim#folding#foldexpr()
  "\ foldtext=lsp#ui#vim#folding#foldtext()
" - pylsp-all: https://github.com/python-lsp/python-lsp-server/blob/develop/CONFIGURATION.md , https://github.com/python-lsp/python-lsp-server
"   Tools enabled by default:
"   - Linter: autopep8
"   - Formatter: pycodestyle
"   - Completion: jedi
"   - Linter alternative: flake8 ruff
"   Ruff: https://github.com/python-lsp/python-lsp-ruff , https://docs.astral.sh/ruff/
"   Our setup disable all linters and code formatters to set Ruff as our
"   default linter
"   Jedi is used to analise decvalrations, reference, autocomplete, hover, etc
"   ...
let g:lsp_settings = {
            \   'pylsp-all': {
            \     'workspace_config': {
...
" NOTE: At the moment use ctags as provider for Vista. vim_lsp doen't display
" symbols in context, hirarchicaly. I will need to test COC at some point
...
" ALE {{{2
" TODO: setup ALE linter and fixer for pyhon usinhg ruff if available, othersie
" fallback to the default
" Other file formats linters? vim? bash?
if  executable('ruff')
    " Linters
...
if g:tdvim_dev_mode
    ...
    " Prepare session magement for project
    ...
    if s:has_git
        ...
        if len(s:root) == 0
            echoerr "Can't guess project root loacation using git. Is this project controlled by Git?"
        else
            if s:GetOS() == 3 && executable('realpath') && executable('cygpath')
            "if s:GetOS() == 3
                " Fix path for windows
                let gitpath = system('git rev-parse --show-cdup')
                if len(gitpath) == 1
                    let gitpath = '.'
                else
            ...
            if !empty(g:tdvim_proj_name)
                " Opening  project message
                autocmd VimEnter * nested  echomsg "Opening project " . g:tdvim_proj_name
            endif
            if s:has_tdvim_root
                let s:tdvim_proj_config = g:tdvim_proj_root_config . "/project.vim"
                if !filereadable(expand(s:tdvim_proj_config))
                    echomsg "Create project.vim"
            ...
            autocmd BufRead * let b:tdvim_proj_root = g:tdvim_proj_root
            autocmd BufRead * exe "lcd " . g:tdvim_proj_root 
    ...
    " on enter
    " To debug session loading dont use silent! con calling the functions
    autocmd VimLeave * call TDVimSaveSess()
    autocmd VimEnter * nested silent! call TDVimRestoreSess() | echomsg "Project session restored"
    ...
    " FZF Commands
    command! -bang ProjectFiles call fzf#vim#files(g:tdvim_proj_root, fzf#vim#with_preview(), <bang>0)
    " Disable autochdir:
    set noautochdir
    " Enable Autowrite:
    set autowriteall
    ...
    " Remap S-F3 if using FZF to use ProjectFiles command instead of Files
    if s:has_fzf
        nunmap <S-F3>
        vunmap <S-F3>
        iunmap <S-F3>
        nnoremap   <S-F3>   :ProjectFiles<CR>
        vnoremap   <S-F3>   :ProjectFiles<CR>
        inoremap   <S-F3>   <ESC>:ProjectFiles<CR>
    endif
    " Git keymaps:
    " Navigate hunks
    ...
    " - Diff  with index
    nnoremap   <F9>   :call TDVimGitDiffToggle() <CR>
    " - Fold Hunks
    ...
    " Symbols keymaps. In dev mode use Vista instead of FzfFunky or simple ctags
    if s:has_fzf
        nunmap <F6>
        xunmap <F6>
        nnoremap   <F6>   :Vista finder<CR>
        xnoremap   <F6>   :Vista finder<CR>
    endif
    nnoremap   <S-F8>   :Vista!!<CR>
    " Enable LSP Mappings for those filetypes for what we usually have a LS
    "autocmd FileType python call s:TDVimLSPMappings()
    "Scratch Pad
    nnoremap   <S-F11>   :Scratch<CR>
    nnoremap   <C-F11>   :ScratchPreview<CR>

    " ALE Fixer and formatter
    if exists("g:ale_enabled")
        " Check if F6 is mapped and unmap it
        if maparg('<S-F6>') != ''
            execute 'unmap <S-F6>'
        endif
        nnoremap   <S-F6>   :ALEFix<CR>
    endif
